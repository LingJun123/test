...
class Model(torch.nn.Module):
    def __init__(self, n_feature, n_class):
        ...
        self.base = nn.Sequential(
            ...
            # 在时间维度上做卷积，T=750，即在750上操作，而非在1024上操作
            nn.LeakyReLU()
            # nn.Conv1d(outchal, outchal, kernel_size=1, stride=1, padding=0),
            # # 在时间维度上做卷积，T=750，即在750上操作，而非在1024上操作
            # nn.LeakyReLU()
        )
    ...
    def forward(self, inputs, device='cpu', is_training=True, points_feature=None, seq_len=None):
        ...
        base_x_f = self.base(inputs[:, :, 1024:].permute(0, 2, 1)).permute(0, 2, 1)  # [24,750,1024]
        base_x_r = self.base(inputs[:, :, :1024].permute(0, 2, 1)).permute(0, 2, 1)
        ...
        att_r = self.softmaxd1(self.att_r(base_x_f.permute(0,2,1)).permute(0,2,1))
        ...
        count_feat = None
        if is_training:
            return cls_x_f, cls_x_r, tcam,score_f,score_r,global_score,att_f,att_r,att_weight_all,act_f,act_r,act_all
        else:
